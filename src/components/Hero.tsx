...
export const Hero = () => {
  const { trackButtonClick, trackCTAClick } = useMixpanel();
  ...
  return (
    <section ref={sectionRef as React.RefObject<HTMLElement>} id="hero" className="relative w-full overflow-hidden font-poppins">
      <BackgroundBeamsWithCollision className="min-h-[500px] pt-6 pb-16 sm:pt-10 sm:pb-20">
        <div className="relative z-10 w-full flex flex-col items-center px-4 max-w-md mx-auto">
          ...
          <div className="flex flex-col items-center text-center">
            <h1 className="font-oi sm:text-3xl leading-none tracking-tight mb-2 text-center text-4xl">
              <span className="text-primary">أحمد</span>{" "}
              <span className="text-[#1A1349]">مانجو</span>
            </h1>
            <h2 className="text-base sm:text-lg leading-tight mb-2 font-lalezar">
              ...
              <span className="text-[#1A1349] text-3xl font-black">ولا فستكة؟</span>
            </h2>
          </div>
          ...
          <div className="flex flex-col justify-center items-center gap-3 mt-4 mb-4 sm:mb-6 w-full">
            {showGameButton ? (
              <>
                <Button
                  onClick={() => {
                    trackCTAClick('العب واكسب جوائز', 'kharbsh_game', 'hero');
                    window.open('https://www.kharbsh.com/ahmed-mango/campaign/3775071ae46c47f4b41f5688e66406a0', '_blank');
                  }}
                  className="h-11 px-6 w-[180px] text-sm font-black rounded-lg text-accent-foreground border-2 border-foreground shadow-bold hover:shadow-bold-sm active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all duration-150 uppercase tracking-wide bg-[#1a1349]"
                >
                  العب واكسب جوائز
                </Button>
                <Button
                  variant="outline"
                  className="h-11 px-6 w-[180px] text-sm font-black rounded-lg bg-background border-2 border-foreground shadow-bold hover:shadow-bold-sm active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all duration-150 uppercase tracking-wide"
                  onClick={() => {
                    trackCTAClick('شوف المراجعات', '#products', 'hero');
          ...
          {/* Mini section nav - scroll indicator */}
          <div className="flex items-center gap-3 text-xs sm:text-sm font-bold text-foreground/60 mt-2">
            <button onClick={() => scrollToSection("#products")} className="hover:text-primary transition-colors">المراجعات</button>
            <span>·</span>
            <button onClick={() => scrollToSection("#latest-content")} className="hover:text-primary transition-colors">الفيديوهات</button>
            <span>·</span>
            <button onClick={() => scrollToSection("#social")} className="hover:text-primary transition-colors">السوشيال</button>
          </div>
        </div>
