...
export const SocialFeeds = () => {
  return (
    <section id="feeds" className="py-10 sm:py-16 px-3 sm:px-4 bg-muted/30">
          ...
          <Card className="p-4 sm:p-6 border-2 hover:border-pink/30 transition-colors">
            ...
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2 mb-4 sm:mb-5">
              {instagramPosts.map((post) => (
                <div 
                  key={post.id}
                  className="aspect-square bg-muted rounded-lg overflow-hidden hover:opacity-80 transition-opacity cursor-pointer active:scale-95"
                >
                  <img 
                    src={post.thumbnail}
                    alt="Instagram post"
            ...
            <Button 
              className="w-full rounded-full font-bold bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 text-white hover:opacity-90 text-xs sm:text-sm h-10 sm:h-11"
          ...
          <Card className="p-4 sm:p-6 border-2 hover:border-secondary/30 transition-colors">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-2 gap-1.5 sm:gap-2 mb-4 sm:mb-5">
              {tiktokPosts.map((post) => (
                <div 
                  key={post.id}
                  className="aspect-[9/14] sm:aspect-[9/16] bg-muted rounded-lg overflow-hidden hover:opacity-80 transition-opacity cursor-pointer active:scale-95"
                >
                  <img 
                    src={post.thumbnail}
                    alt="TikTok video"
            ...
            <Button 
              className="w-full rounded-full font-bold bg-black text-white hover:bg-black/90 text-xs sm:text-sm h-10 sm:h-11"
          ...
          <Card className="p-4 sm:p-6 border-2 hover:border-red-500/30 transition-colors sm:col-span-2 lg:col-span-1">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-1 lg:grid-cols-1 gap-1.5 sm:gap-2 mb-4 sm:mb-5">
              {youtubePosts.map((post) => (
                <div 
                  key={post.id}
                  className="aspect-video bg-muted rounded-lg overflow-hidden hover:opacity-80 transition-opacity cursor-pointer active:scale-95"
                >
                  <img 
                    src={post.thumbnail}
                    alt="YouTube video"
            ...
            <Button 
              className="w-full rounded-full font-bold bg-red-600 text-white hover:bg-red-700 text-xs sm:text-sm h-10 sm:h-11"
